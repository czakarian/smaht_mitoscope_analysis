...
TABLE_BY_SAMPLE="/net/nwgc/vol1/home/czaka/analysis/mitoscope/smaht/benchmark_analysis/vcfeval_scripts/vcfeval_table_by_sample_v2.csv"

## tables by sample
ALL_OUT_BY_SAMPLE="${INDIR}/combined_tables/HPRC.vcfeval.by_sample.all.v2.VAF025.tsv"
SNV_OUT_BY_SAMPLE="${INDIR}/combined_tables/HPRC.vcfeval.by_sample.snvs.v2.VAF025.tsv"
INDEL_OUT_BY_SAMPLE="${INDIR}/combined_tables/HPRC.vcfeval.by_sample.indels.v2.VAF025.tsv"
...
do
    ## snvs
    x=$(zcat ${INDIR}/${tool}/${tech}_025/${sample}/snp_roc.tsv.gz | grep -v '^#' | cut -f 2- || true)
    echo -e "${tool}\t${tech}\t${sample}\t${x}" >> ${SNV_OUT_BY_SAMPLE}

    ## indels
    x=$(zcat ${INDIR}/${tool}/${tech}_025/${sample}/non_snp_roc.tsv.gz | grep -v '^#' | cut -f 2- || true)
    echo -e "${tool}\t${tech}\t${sample}\t${x}" >> ${INDEL_OUT_BY_SAMPLE}

    ## all 
    x=$(zcat ${INDIR}/${tool}/${tech}_025/${sample}/weighted_roc.tsv.gz | grep -v '^#' | cut -f 2- || true)
