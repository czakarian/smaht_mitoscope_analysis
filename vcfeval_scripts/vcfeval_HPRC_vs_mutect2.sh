...
INDIR="/net/nwgc/vol1/home/czaka/analysis/${tool}/smaht/${tech}/hapmap/"
SAMPLES=$(cut -d',' -f 1 ${INDIR}/samples.csv | tail +2)
MULTISAMPLE_VCF="${INDIR}/output/merged.mutect2.filtered.norm.vcf.gz"
MULTISAMPLE_FORMATTED_VCF="${INDIR}/output/merged.mutect2.filtered.norm.vcfeval.vcf.gz"
...
bcftools norm --multiallelics -both ${MULTISAMPLE_VCF} | \
bcftools +fill-tags -- -t 'NUM_AC:1=AC' | \
bcftools +fill-tags -- -t 'AVG_HET:1=sum(AF)/INFO/NUM_AC' | \
bcftools +fill-tags -- -t 'MAX_HET:1=MAX(FORMAT/AF)' | \
...
# do
#     echo ${SAMPLE}
#     INPUT_VCF="${INDIR}/output/${SAMPLE}/${SAMPLE}.mutect2.filtered.norm.vcf.gz"
#     FORMATTED_VCF="${INDIR}/output/${SAMPLE}/${SAMPLE}.mutect2.filtered.norm.vcfeval.vcf.gz"
