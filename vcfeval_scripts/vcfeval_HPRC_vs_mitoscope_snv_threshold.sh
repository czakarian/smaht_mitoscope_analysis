...
tech="pacbio"
tool="mitoscope"
categories="_snv1 _snv05 _snv025 _snv01"

INDIR="/net/nwgc/vol1/home/czaka/analysis/${tool}/smaht/hapmap/${tech}/"
SAMPLES=$(cut -d',' -f 1 ${INDIR}/samples.csv)

for category in ${categories[@]}; do

    MULTISAMPLE_VCF="${INDIR}/output${category}/merged.mt.baldur.annotated.vcf.gz"
    MULTISAMPLE_FORMATTED_VCF="${INDIR}/output${category}/merged.mt.baldur.annotated.vcfeval.vcf.gz"
    ...
    do
        echo ${SAMPLE}
        INPUT_VCF="${INDIR}/output${category}/${SAMPLE}/variants/baldur/${SAMPLE}.mt.baldur.annotated.vcf.gz"
        FORMATTED_VCF="${INDIR}/output${category}/${SAMPLE}/variants/baldur/${SAMPLE}.mt.baldur.annotated.vcfeval.vcf.gz"
