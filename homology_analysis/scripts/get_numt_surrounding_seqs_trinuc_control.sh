...
tail +2 ${infile} | while IFS="," read -r line; do
    ...
    mito_outer_left=$(samtools faidx ${MT_REF} MT:$((${mito_start} - ${flank} - ${offset}))-$((${mito_start} - 1 - ${offset})))
    mito_inner_left=$(samtools faidx ${MT_REF} MT:$((${mito_start} + ${offset}))-$((${mito_start} + ${flank} - 1 + ${offset})))
    mito_outer_right=$(samtools faidx ${MT_REF} MT:$((${mito_end} + 1 + ${offset}))-$((${mito_end} + ${flank} + ${offset})))
    mito_inner_right=$(samtools faidx ${MT_REF} MT:$((${mito_end} - ${flank} + 1 - ${offset}))-$((${mito_end} - ${offset})))
